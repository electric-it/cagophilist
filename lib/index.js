...
function cago(cmd1, cmd2) {
  return new Promise((finalResolve, finalReject) => {
    ...
    const runCommands = ['update', 'refresh', 'exclude', 'setup', 'list'];
    ...
    Promise.resolve()
      ...
      .then(() => {
        if (
          ...
          printHelp.push(`\t   ${chalk.green('list     ')}\t List the available profiles to refresh, update or assume`);
      ...
      .then(() => new Promise((resolve, reject) => {
        pathUtils.verifyPaths(cagoOptions)
          ...
          .then((ret) => {
            cagoOptions.registeredHooks = ret.registeredHooks || [];
            resolve();
          })
          .catch((err) => {
            logger.error(chalk.red(`\n${err}`));
      ...
      .then(() => {
        if (cagoOptions.registeredHooks.indexOf('version-checks') !== -1) {
          // the check versions from a plugin
          return pluginUtils.runPlugins(cagoOptions, 'version-checks')
      ...
      .then(() => {
        switch (cmd1) {
          case 'list':
            return listCmd.run(cagoOptions);
